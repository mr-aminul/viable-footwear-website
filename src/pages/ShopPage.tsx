...
export function ShopPage() {
  ...
  const filtered = useMemo(() => {
    let list = getProductsByCategory(category === 'all' ? undefined : category)
      ...
      {filtered.length === 0 && (
        <div className="py-20 text-center">
          ...
          <button
            type="button"
            onClick={() => setParams({})}
            className="mt-4 text-[14px] font-semibold text-navy underline-offset-4 hover:underline"
          >
            Clear filters
          </button>
        </div>
  ...
  return (
    <button
      ...
      className={`shrink-0 rounded-full px-4 py-2 text-[13px] font-medium transition ${
        active
          ? 'bg-navy text-white'
          : 'bg-mist text-ink hover:bg-cloud'
      }`}
    >
      {label}
    </button>
